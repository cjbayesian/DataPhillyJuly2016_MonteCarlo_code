...
target_sd=1,seed=1,iter=5000,plot_file='MH.pdf')
{
   ...
   pdf(plot_file)
   par(mfrow=c(3,1),mgp = c(1, 0.5, 0),cex=1.1)
      ...
      for(i in 1:iter)
      {
         ...
         curve(dnorm(x,target_mu,target_sd),
            col='blue',lty=2,
            ylim=c(0,1.2*dnorm(0,0,prop_sd)),
            lwd=2,xlim=plot_range,
            ylab='Target Distribution')
         if(i > 2)
            lines(density(track,adjust=1.5),col='red',lwd=2)

         points(track,rep(0,i),pch=3)
         ...
         curve(dnorm(x,target_mu,target_sd),
            ...
            ylim=c(0,1.2*dnorm(0,0,prop_sd)),
            col='blue',lty=2,
            ylab='Metropolis-Hastings',lwd=2)
